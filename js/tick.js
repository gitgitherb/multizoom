...
function tick(event) {
  ...
  if (statusGame > 0) {
    ...
    if (statusFriends == 'jump') {
      // 上昇〜下降
      spriteChar.y -= jumpPower;// Y座標の移動
      jumpPower -= GRAVITY;// ジャンプ力の重量修正
      // debugText.text = 'jump! '+spriteSaba.y;
      // debugText.text = 'jump! '+jumpPower/2;

      if (spriteChar.y > GROUND_LINE - CHAR_H) {
        // 着地
      ...
      // ダッシュ
      spriteChar.x += friendsList[activeFriends]['dashSpeed'];
      if (spriteChar.x > (STAGE_W / 2) - (CHAR_W / 2)) {
        // 画面中央に戻ったらrunに戻る
        spriteChar.x = (STAGE_W / 2) - (CHAR_W / 2);
        statusFriends = 'run';
        spriteChar.gotoAndPlay(friendsList[activeFriends]['name']+"_run");// 走るアニメに変更
      }
    } else if (statusFriends == 'damage') {
      // ダメージ
      spriteChar.x -= friendsList[activeFriends]['speed'];
      if (spriteChar.x < 0) {
        // ある程度したらダッシュ状態に移行
        statusFriends = 'dash';
